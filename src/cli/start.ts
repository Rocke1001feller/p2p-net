 ...
 *  - 长驻进程：stop() 反向收尾全部组件（配对环/HostAgent/隧道/scanner/两个 http server）。
 */
...
import { createScanner, DEFAULT_WHITELIST, NEVER_PORTS, type ServiceInfo } from '../server/scanner.js';
...
export async function runStart(opts: RunStartOptions = {}, deps: RunStartDeps = {}): Promise<StartHandle> {
  ...
  const discovery = startDiscoveryFn({ log, getServices: () => scanner.list(), deviceId: () => deviceId });
