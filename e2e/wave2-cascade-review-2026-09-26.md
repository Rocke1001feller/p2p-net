...
**2026-09-26 更新（强候选根因已实证）**：W2-6 spike 期间 coturn syslog 抓到 ALLOCATE 失败 `error 508 Cannot create socket`——relay 端口池仅 20（50000-50019），耗尽即拒绝新分配，且每分配残留 ~600s，快速迭代必撞池。与本发现「快速迭代间分配竞争」假设方向一致，升为强候选根因。取证详见 `e2e/wave2-upgrade-wheel-spike-2026-09-26.md` §2.3；产能约束已入 `docs/cost-model.md` §6.1「TURN 并发上限（coturn）」行。
